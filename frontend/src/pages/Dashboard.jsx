...
export default function Dashboard() {
  ...
  const totalIssues = analytics?.summary?.totalDefects ?? issues.length;
  const openIssues = analytics?.summary?.openDefects ?? issues.filter((i) => i.status === "Open").length;
  const inProgressIssues = analytics?.summary?.inProgressDefects ?? issues.filter((i) => i.status === "In Progress").length;
  const inReviewIssues = analytics?.summary?.inReviewDefects ?? issues.filter((i) => i.status === "In Review").length;
  const resolvedIssues = analytics?.summary?.resolvedDefects ?? issues.filter((i) => i.status === "Resolved").length;
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              ...
              >
                {hotspotLoading ? "Analyzing..." : "▶ Run Hotspot AI"}
              </button>
            </div>
          </div>
          ...
          {hotspotLoading ? (
            ...
            <p style={{ color: "#75676a", textAlign: "center", padding: "15px" }}>
              Click <strong>"Run Hotspot AI"</strong> above to analyze codebase vulnerability and module fragility.
            </p>
          )}
        </div>
